...
export const assignUserToPlanProcedure = async (planProcedureId, userId) => {
  const url = `${api_url}/planprocedure/assign-user`;
  const command = { planProcedureId, userId };
  ...
  if (!response.ok) throw new Error("Failed to assign user to plan procedure");
